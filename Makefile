...
init-db: ## Инициализировать структуру базы данных (без тестовых данных)
	@echo "$(GREEN)Инициализация базы данных...$(NC)"
	@$(VENV_BIN)/python $(BACKEND_DIR)/init_prod_db.py
...
docker-up: ## Запустить все сервисы в Docker
	...
	@$(DOCKER_COMPOSE) up -d || { \
		echo "$(YELLOW)⚠ Ошибка при запуске. Проверьте конфигурацию.$(NC)"; \
		exit 1; \
	}
...
docker-init-db: ## Инициализировать базу данных в Docker
	@echo "$(GREEN)Инициализация базы данных...$(NC)"
	@$(DOCKER_COMPOSE) exec backend python init_prod_db.py || \
	 (echo "$(YELLOW)⚠ Контейнер не запущен. Запустите сначала: make docker-up$(NC)" && exit 1)
	@echo "$(GREEN)✓ База данных инициализирована$(NC)"
